...
function updateTimer(jobId, startDate, endDate) {
    ...
    if (remainingTime <= 0) {
        clearInterval(job.timerInterval); // Ensure the interval is cleared
        ...
        if (phoneDiv) {
            ...
            // Assuming openWhatsApp is a function defined elsewhere
            // phoneInput.removeEventListener('click', openWhatsApp); // Uncomment if openWhatsApp is defined
        }

        // Apply blur and disable tooltips
        ...
        // Check if the job is new within the first 30 minutes after the start date
        const timeSinceStart = now - start;
        if (timeSinceStart <= 12 * 60 * 60 * 1000) { // 12 hours in milliseconds
    ...
    job.timerInterval = setInterval(function() {
        updateTimer(jobId, startDate, endDate); // Recursively call the function to update the timer
    }, 1000);
}

// Função para gerar listagens de empregos com funcionalidades de telefone e email
function generateJobListingsWithPhoneAndEmail() {
    ...
    entries.forEach(entry => {
        ...
        // Criação do HTML para cada listagem de emprego
        ...
        // Adiciona event listener ao botão "Candidatar-se"
        ...
        // Configura o timer para esta listagem de emprego
...
// Função para alternar a visibilidade do input de email
...
// Função para alternar a visibilidade do input de telefone
...
// Chama a função para gerar listagens de empregos
